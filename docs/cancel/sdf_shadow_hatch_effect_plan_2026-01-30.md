...
- [x] `src/grafix/core/effects/sdf_shadow_hatch.py` を追加（effects/AGENTS.md を遵守）
- [x] `@effect(meta=..., n_inputs=1)` で登録（meta に `activate` は入れない）
- [x] mask 基準で平面整列（util 使用）＋ planarity 最小チェック
- [x] mask からリング抽出（閉曲線のみ採用）
- [x] Numba で SDF グリッド評価
- [x] Numba で bilinear サンプラ
- [x] Numba で shadow_grid（ray marching）
- [x] shadow_grid をスキャンしてハッチ線分生成（回転→水平スキャン→逆回転）
- [x] `src/grafix/core/builtins.py` にモジュール追加
- [x] `src/grafix/api/__init__.pyi` を生成結果で更新
- [x] `tests/core/effects/test_sdf_shadow_hatch.py` を追加
- [x] `PYTHONPATH=src pytest -q`（全体では別テストが 1 件失敗 / 本 effect のテストは通過）
