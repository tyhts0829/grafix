...
- [ ] 入力の取り方
  - 案 A: `n_inputs=1` で `inputs[0]` 内の複数ポリライン（閉曲線）を対象（推奨）
  - 案 B: `n_inputs=2`（`inputs[0]` と `inputs[1]` を接続して返す）も同時に対応
- [ ] アルゴリズム（どちらで行く？）
...
- [ ] 閉曲線判定/自動クローズ
  - 案 A: 端点距離 `<= auto_close_threshold` なら閉じる（`buffer` と同様の体験、推奨）
...
### 3.3 “メタボール的接続”本体（案A: Shapely closing）
...
（案B: Marching Squares を採用する場合は、この節を差し替え）
