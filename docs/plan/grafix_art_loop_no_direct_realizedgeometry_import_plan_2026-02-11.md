...
- [ ] 組み込み primitive/effect 一覧 + 1行説明セクションを作る
  - primitives 全件
  - effects 全件
  - 説明は「何をするオペレーションか」の1行に統一
- [ ] 一覧の更新ルール（更新元・更新手順）をガイド末尾に明記する

## 2) スキル文言と参照順の更新

- [ ] `.agents/skills/grafix-art-loop-artist/SKILL.md` を更新する
  - 新規ガイドを最初に参照する導線を追加
  - `grafix.core.realized_geometry` 直import禁止を明記
- [ ] `.agents/skills/grafix-art-loop-orchestrator/SKILL.md` を更新する
  - artist 実行前の参照順に新規ガイドを追加
  - 同じ禁止ポリシーを明記
- [ ] `.agents/skills/grafix-art-loop-orchestrator/references/project_quick_map.md` に新規ガイドへのリンクを追加する
...
- [ ] Layer 指定のサンプルがガイド通り動くことを確認（少なくとも1例）

## 6) 完了条件

- [ ] artist向け新規ガイドが追加され、参照導線が skills/quick_map から辿れる
- [ ] ガイドに primitives/effects 全件の1行説明が載っている
- [ ] docstring 取得 MCP ツールが実装され、名前指定で取得できる
