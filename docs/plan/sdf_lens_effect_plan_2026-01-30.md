...
- 実装:
  ...
  - `@njit(cache=True, parallel=True)` で `_evaluate_sdf_points(...) -> d` を実装
    - 全線分への最短距離（ユークリッド）
...
- [ ] `src/grafix/core/effects/sdf_lens.py` を追加（effects/AGENTS.md を遵守）
- [ ] `@effect(meta=..., n_inputs=2)` で登録（`ui_visible` で kind ごとに表示制御）
- [ ] lens 基準で平面整列（util 使用）＋ planarity 最小チェック
- [ ] lens リング抽出（閉曲線のみ採用）
- [ ] Numba で `signed distance` を点列に対して評価
- [ ] `profile/band/inside_only` に従って weight を計算し、`kind` で変形を適用
- [ ] `src/grafix/core/builtins.py` にモジュール追加
- [ ] `src/grafix/api/__init__.pyi` を生成結果で更新
- [ ] `tests/core/effects/test_sdf_lens.py` を追加
