...
- [ ] クリア対象の範囲
  - 案A: **ParamStore 内の全キー**（GUI 非表示グループも含めて `cc_key` を全解除）
  - 案B: **GUI 表示中の行のみ**（`store_snapshot_for_gui()` の範囲のみ）
- [ ] `runtime.last_effective_by_key` が無いキー（= 解除時点の effective が取れない）の扱い
  - 案A: **bake をスキップして `cc_key` のみ外す**（稀に値がジャンプし得る）
  - 案B: resolver を呼び、**解除時点 effective を計算して bake**（実装範囲が広がる）
- [ ] ボタン文言/配置（例: 上部に `Clear MIDI Assigns` を 1 個）

## 1) 受け入れ条件（完了の定義）

- [ ] ボタン押下で、対象範囲の全パラメータについて `state.cc_key is None` になる
- [ ] 解除前に MIDI で動いていたパラメータが、解除直後に **値ジャンプしない**
- [ ] 解除後は MIDI CC 入力で値が変化しない（= 割当が消えている）
- [ ] MIDI learn 中だった場合は解除時に learn がキャンセルされる（意図せぬ即時再割当を防ぐ）
- [ ] `PYTHONPATH=src pytest -q tests/interactive/parameter_gui/test_parameter_gui_cc_unassign_bake.py`
- [ ] `PYTHONPATH=src pytest -q tests/interactive/parameter_gui -k \"clear_all|midi_assign\"`（新規/追加テスト）
...
- [ ] `tests/interactive/parameter_gui/test_parameter_gui_clear_all_midi_assignments.py`（新規）
...
- [ ] 0) の事前確認を確定（対象範囲 / effective 無しの扱い / ボタン文言）
- [ ] `store_bridge.py` に一括解除関数を追加（テストから先に呼べる形）
- [ ] テスト追加（新規ファイル）→ 期待仕様が満たせることを固定
- [ ] `gui.py` にボタンを追加し、押下で一括解除 + learn キャンセル
- [ ] 対象テストを実行して最小確認
