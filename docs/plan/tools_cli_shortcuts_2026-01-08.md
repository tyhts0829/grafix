...
# 何を: ベンチ計測+レポート生成/スタブ生成を「配布パッケージに含めた上で」 `python -m grafix benchmark` / `python -m grafix generate_stub` で実行できる導線を追加する計画。
...
- スタブ生成を短いコマンドで実行できる:
  - `python -m grafix generate_stub` → `grafix.api.__init__.pyi` 再生成（旧: `tools/gen_g_stubs.py` 相当）
...
- 「開発用コマンド」を増やしすぎる（最小の 2 系統: benchmark / generate_stub に絞る）。
...
- スタブ生成コマンドは `python -m grafix generate_stub` にする。
...
- [ ] 入口 CLI を追加
  ...
  - [ ] サブコマンド `benchmark` と `generate_stub` を定義する
- [ ] ベンチ: CLI を接続
  - [ ] `python -m grafix benchmark ...` が `effect_benchmark.main(argv)` → `generate_report.main()` を順に実行する
- [ ] スタブ: CLI を接続
  - [ ] `python -m grafix generate_stub` が `grafix.devtools.generate_stub.main()` を実行する
- [ ] テスト/参照の移行
...
- [ ] ドキュメント（最小）
  ...
  - [ ] `docs/memo/generate_stub.md` を `python -m grafix generate_stub` に更新する
- [ ] 検証（手元コマンド）
  ...
  - [ ] `python -m grafix generate_stub` で `grafix/api/__init__.pyi` が更新される（editable install 前提）
...
- [ ] `python -m grafix benchmark` / `python -m grafix generate_stub` が迷わず実行できる
