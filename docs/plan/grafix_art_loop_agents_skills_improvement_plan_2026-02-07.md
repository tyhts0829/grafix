...
- [x] role skills に分散していた「計画 md 不要」などの運用ルールを orchestrator に集約する（読み手の迷いを減らす）

### 1) スキーマ拡張：design tokens と差分指示を “構造” にする

- [x] `.agents/skills/grafix-art-loop-orchestrator/references/schemas.md` を拡張し、最低限次を追加する
...
- [x] `Artifact.params` に `design_tokens_used`（実際に使ったトークン）を入れる運用を固定する
- [x] `Critique.winner` に「ロック/可変」を明示できるフィールドを追加する（例）
...
- [x] `.agents/skills/grafix-art-loop-ideaman/SKILL.md` を更新し、`CreativeBrief` の必須項目として
  - 構図テンプレ（composition_template）
  - design tokens（固定/レンジ/候補）
  - layers（主役/副要素/テクスチャの役割）
    を要求する
- [x] `variation_axes` は「token をどう動かすと画がどう変わるか」に直接対応する文で書かせる（token 名を含める）
- [x] “完全自由”を避け、パレット/線/間隔/ノイズ粒度は **少数候補**から選ばせる（探索がノイズ化しない）

### 3) artist skill 改善：コード側を「デザインのレバーがある構造」にする

- [x] `.agents/skills/grafix-art-loop-artist/SKILL.md` を更新し、実装規約として
...
- [x] iteration ごとの `explore_ratio`（例: 序盤 0.7 → 終盤 0.2）を想定し、各 variant に `mode`（exploration/exploitation）を付与する運用を `SKILL.md` に明記する
- [x] exploration variant には baseline/critic_feedback を渡さない運用を `SKILL.md` に明記する（ロックで即収束しない）
- [x] winner 情報（artifact と critic 指示）を次反復へ引き継ぐ運用を `SKILL.md` に明記する
...
- design tokens（最小キーセット）:
  - tokens: `composition_template` / `vocabulary` / `palette` / `stroke` / `spacing` / `noise` / `grid_unit`
  - tokens とは別フィールド: `layers`（`hero` / `support` / `texture`）
- exploration/exploitation:
  - `explore_ratio` を **0.7 → 0.2** に線形減衰（`N` に追従）
  - 各 iteration の `M` 本を `ceil(M*explore_ratio)` 本だけ exploration、残り exploitation（exploitation は最低 1 本）
  - exploration は `composition_template` / `vocabulary` の変更 OK、exploitation は `locked_tokens` 固定で `spacing` / `noise` / `stroke` の微調整中心
- 今回のスコープ:
  - やる: tokens+layers の明文化、critic の `locked_tokens` / `mutable_tokens` + 「最大 3 件の差分 directive（token 指定 + success_criteria）」、orchestrator の `mode` 付与
  - 見送り: 選好学習（Ask-first）
  - 保留: 破綻フィルタ（まず計測→JSON 化から）、レンダ品質（tokens が固まってから）
