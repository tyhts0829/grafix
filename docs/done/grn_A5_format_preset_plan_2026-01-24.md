# grn/a5_frame.py の preset 化（実装計画）

## 目的

`sketch/presets/grn/a5_frame.py` を「再利用できる preset」として切り出し、他のスケッチから `P.<name>(...)` で呼び出せるようにする。

## 完了条件

- `P.grn_a5_frame(...)` を呼ぶと、現状の A5 フォーマット（layout + template）が同等に生成される。
...
- `sketch/presets/grn/a5_frame.py` は内部で `P.layout_grid_system(...)` を呼んでいる。
  - このファイル自体を `@preset` 化すると、**直接実行**（`python sketch/presets/grn/a5_frame.py`）時に `P` の autoload が走って同一 preset を二重登録し、例外になる可能性がある。
  - 対応方針はどちらか:
...
- [x] 1. `sketch/presets/grn/a5_frame.py` に `meta` と `@preset` 付き関数（名前は合意したもの）を追加する
