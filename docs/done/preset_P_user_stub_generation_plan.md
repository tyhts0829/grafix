...
# 何を: `python -m grafix stub` で、ユーザー定義 preset を `P.<name>` の明示メソッドとして列挙し、IDE 補完（引数名/型）を効かせる。
...
- ユーザーは「preset を追加/変更したら `python -m grafix stub`」だけでよい（手動 import 不要）。
...
- `stub` 実行時に preset autoload を走らせ、レジストリに登録された callable から `_P` を生成する。
...
- [x] 生成後の `src/grafix/api/__init__.pyi` を更新する
  - [x] `python -m grafix stub` を実行して同期する
- [x] テストを追加する（回帰防止）
...
- [x] README を最小更新する
  - [x] 「ユーザー preset の補完は `preset_module_dirs` + `python -m grafix stub`」を 2〜3 行で追記する

## 追加で気づいた点（提案）

- `stub` は “import 副作用” を許容するコマンドなので、preset autoload をここで走らせるのが一番素直。
- autoload 由来の import エラー/二重登録（同名 preset）は、スタブ生成を失敗させて早期に気づける方が安全。
- `stub` 側で preset 一覧を列挙するため、`PresetFuncRegistry.items()`（read-only）を追加した。
