...
def test_sync_slider_scenario_reports_phases_progress_and_exact_output() -> None:
    result = run_interactive_slider_scenario(
        make_interactive_slider_scenario(_sync_parameters())
    )

    assert result.value["scope"] == (
        "hosted:fake-gui+scene-runner+fake-gl+present-marker"
    )
    assert result.value["final_input_revision_delta"] == 6
    assert (
        result.value["final_presented_revision_delta"]
        == result.value["final_input_revision_delta"]
    )
    assert (
        result.value["final_mesh_checksum"]
        == result.value["expected_final_mesh_checksum"]
    )
    assert all(
        contract.passed
...
def test_one_worker_slider_scenario_reaches_the_final_revision() -> None:
    ...
    result = run_interactive_slider_scenario(
        make_interactive_slider_scenario(parameters)
    )
...
def test_hosted_renderer_does_not_advance_scene_serial_for_stale_redisplay(
    ...
    assert (
        metrics["ux01.scene_serial_advances"]
        == metrics["ux01.fresh_present_frames"]
    )
    contract = next(
        item
