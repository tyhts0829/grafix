...
def test_effect_defaults_recorded_when_no_kwargs():
    ...
    assert scale_args == {"activate", "mode", "auto_center", "pivot", "scale"}
...
def test_meta_dict_spec_is_accepted_for_user_defined_primitive_and_effect() -> None:
    ...
    @primitive(meta=meta)
    def user_defined_meta_dict_primitive(*, x: float = 1.0) -> GeomTuple:
        _ = x
        return _empty_geometry()

    @effect(meta=meta)
    def user_defined_meta_dict_effect(g: GeomTuple, *, x: float = 1.0) -> GeomTuple:
        _ = x
        return g
    ...
    assert primitive_args == {"activate", "x"}
    assert effect_args == {"activate", "x"}
