...
def test_primitive_bypass_returns_empty_geometry() -> None:
    base = realize(G.polygon(n_sides=6))
    bypassed = realize(G.polygon(bypass=True, n_sides=6))
...
def test_primitive_bypass_works_without_meta() -> None:
    @primitive(meta=None)
    ...
    bypassed = realize(
        Geometry.create("dummy_primitive", params={"x": 2.0, "bypass": True})
    )
